...
function chcekPrice(inputName) {
  let price = 1250;
  let start = 지하철.indexOf(inputName);
  let end = 지하철.indexOf("역삼");
  let range = start <= end ? end - start : start - end;
  if (start === -1) return "존재하지 않는 역입니다.";
  return range <= 10 ? `${price}원` : `${((range - 10) / 5 + 1) * 100 + price} 원 입니다. `;
}

console.log(chcekPrice("뚝섬"));
